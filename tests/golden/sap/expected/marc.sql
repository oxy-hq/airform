...
stg_sap__marc as (
  select *
  from main_sap.stg_sap__marc
),

stg_sap__t001w as (
  select *
  from main_sap.stg_sap__t001w
),

stg_sap__sapsll_tunos as (
  select *
  from main_sap.stg_sap__sapsll_tunos
),

stg_sap__sapsll_maritc as (
  select *
  from main_sap.stg_sap__sapsll_maritc
),

stg_sap__sapsll_nosca as (
  select *
  from main_sap.stg_sap__sapsll_nosca
),

stg_sap__sapsll_clsnr as (
  select *
  from main_sap.stg_sap__sapsll_clsnr
),
...
nsdm_v_marc_diff as (
  select
    ...
    cast(max(nsdm_v_marc_agg.gjper_max) as VARCHAR) as gjper,
    'x' as mcrue,
    sum(
      case
        nsdm_v_marc_agg.lbbsa
