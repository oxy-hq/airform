...
from final
  ...
  select *
  from "zendesk"."main_zendesk_intermediate"."int_zendesk__reply_time_calendar_hours"


), reply_time_business_hours_sla as (

  select *
  from "zendesk"."main_zendesk_intermediate"."int_zendesk__reply_time_business_hours"
...
-- Now that we have the breach time, see when the first reply after the sla policy was applied took place.
  ...
  select
    *,
    row_number() over (partition by ticket_id, sla_policy_name, metric, sla_applied_at  order by sla_schedule_start_at) as day_index,
    ...
    min(sla_breach_at) over (partition by ticket_id, sla_policy_name, metric, sla_applied_at  order by sla_schedule_start_at rows unbounded preceding) as first_sla_breach_at,
    coalesce(lag(sum_lapsed_business_minutes) over (partition by ticket_id, sla_policy_name, metric, sla_applied_at  order by sla_schedule_start_at), 0) as sum_lapsed_business_minutes_new,
    date_diff('second', sla_schedule_start_at::timestamp, agent_reply_at::timestamp ) / 60 as total_runtime_minutes -- total minutes from sla_schedule_start_at and agent reply time, before taking into account SLA end time
  ...
  select  
    *,
    ...
    case when total_runtime_minutes < 0 -- agent has already replied to prior to this SLA schedule
      ...
      else sum_lapsed_business_minutes_new + (date_diff('second', sla_schedule_start_at::timestamp, coalesce(agent_reply_at, next_solved_at, current_time_check)::timestamp ) / 60) -- otherwise, the sla_elapsed_time will be sum_lapsed_business_minutes_new (the prior record's sum_lapsed_business_minutes) plus the minutes between SLA schedule start and agent_reply_time. If the agent hasn't replied yet, then the minute counter is still running, hence the coalesce of agent_reply_time and current_time_check.
