...
cast(
    ...
    date_add(ticket_status_crossed_with_schedule.status_schedule_start, interval (1) day)

), interval (-1) day)

 as date)as timestamp)::timestamp, cast(ticket_status_crossed_with_schedule.status_schedule_start as timestamp)::timestamp ) /60
          ) as start_time_in_minutes_from_week,
      (date_diff('second', ticket_status_crossed_with_schedule.status_schedule_start::timestamp, ticket_status_crossed_with_schedule.status_schedule_end::timestamp ) /60
