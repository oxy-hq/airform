with base as (
    ...
    select * 
    from "amplitude"."main__source_amplitude"."stg_amplitude__event_tmp"
),
...
, 'amplitude' || '.'|| 'amplitude_integration_tests' as source_relation
...
final as (
    ...
    select
        ...
        md5(cast(coalesce(cast(source_relation as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(user_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(session_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as unique_session_id,
    ...
    where cast(event_time as date) >= cast('2020-01-01' as date) -- filter to records past a specific date
    and cast(event_time as date) <= cast(now() as date) -- filter to records before a specific date
),
