...
session_data as (
    ...
    select *
    from "amplitude"."main_amplitude"."amplitude__sessions"
),
...
final as (
    ...
    select
        *,
        md5(cast(coalesce(cast(source_relation as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(unique_event_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(unique_event_type_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as unique_key
    from event_enhanced
)
