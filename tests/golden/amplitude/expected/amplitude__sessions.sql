with event_data_raw as (
    ...
    select events.*
    from "amplitude"."main__source_amplitude"."stg_amplitude__event" as events
...
session_agg as (
    ...
    select
        ...
        date_diff('second', min(event_time)::timestamp, max(event_time)::timestamp ) / 60 as session_length_in_minutes
    from event_data
    group by 1,2,3
),
...
session_ranking as (
    ...
    select
        ...
        cast(date_trunc('day', session_started_at) as date) as session_started_at_day,
        cast(date_trunc('day', session_ended_at) as date) as session_ended_at_day,
        case
...
select 
    *,
    ...
    case
        when user_id is not null then date_diff('second', last_session_ended_at::timestamp, session_started_at::timestamp ) / 60
        else null
